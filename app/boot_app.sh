...
winpty docker run -it --env AWS_ACCESS_KEY_ID --env AWS_SECRET_ACCESS_KEY -p 3000:3000  --mount type=bind,source="$(pwd)/data",target=/app/data --name testapp mbti_app app.py